...
class MultiModalSubModularExplanation(object):
    ...
    def __call__(self, image_set, id = None):
        ...
        source_image = self.preproccessing_function(self.org_img) #Hàm transform_vision_data chuyển ảnh thành tensor

        self.source_feature = self.model(source_image.unsqueeze(0).to(self.device)) #thêm size cho tensor
        self.target_label = id #id = 0
